...
stapel-social is a composite preset (preset.py wires stapel-chat, stapel-
profiles and stapel-reviews together) with no per-module ``_codegen.py``/
``_capabilities.py`` pipeline: ``docs/capabilities.json`` here is HAND-
AUTHORED (see the "docs: author capabilities.json for the stapel-catalog
sweep" commit), not emitted. There is therefore no triad/capabilities drift
gate to run in this file — that arrives with a codegen pipeline, not before
one.
...
# human half: what this composite is and how to think about it) plus the
# contract documents above (badges, version, doc links). Everything a
